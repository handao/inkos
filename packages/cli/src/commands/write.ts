...
const writeNextCommand = writeCommand
  .command("next")
...
withBackend(writeNextCommand);

writeNextCommand.action(async (bookIdArg: string | undefined, opts) => {
  try {
    ...
    if (be) {
      ...
      for (let i = 0; i < count; i++) {
        if (!opts.json) log(`Writing chapter ${i + 1}/${count}...`);
        const result = await be.backend.writeNextChapter(bookId, {
          wordCount: opts.words ? parseInt(opts.words, 10) : undefined,
        });
        results.push(result);
      }
      if (opts.json) {
        ...
        log(`Wrote ${count} chapter(s) for "${bookId}"`);
      }
      return;
    }
...
const writeRewriteCommand = writeCommand
  .command("rewrite")
...
withBackend(writeRewriteCommand);

writeRewriteCommand.action(async (args: ReadonlyArray<string>, opts) => {
  try {
    const be = getBackendAdapter(opts);
    if (be) {
      const root = findProjectRoot();
      ...
      if (!opts.json) log(`Rewrite not directly supported via backend. Use 'write next' for new chapters.`);
      return;
    }
...
const writeSyncCommand = writeCommand
  .command("sync")
...
withBackend(writeSyncCommand);

writeSyncCommand.action(async (args: ReadonlyArray<string>, opts) => {
  try {
    const be = getBackendAdapter(opts);
    if (be) {
      if (!opts.json) log("Sync not supported via backend. Use the TS core engine (without --backend).");
      return;
    }
...
const writeRepairStateCommand = writeCommand
  .command("repair-state")
  .description("Rebuild truth files for a persisted state-degraded chapter without rewriting body text")
  .argument("<args...>", "Book ID (optional) and chapter number")
  .option("--json", "Output JSON");

withBackend(writeRepairStateCommand);

writeRepairStateCommand.action(async (args: ReadonlyArray<string>, opts) => {
  try {
    const be = getBackendAdapter(opts);
    if (be) {
      if (!opts.json) log("repair-state not supported via backend. Use the TS core engine (without --backend).");
      return;
    }
